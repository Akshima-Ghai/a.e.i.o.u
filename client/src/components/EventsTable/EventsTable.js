    ...
    <TableContainer className={classes.table} component={Paper} elevation={0}>
      ...
      <Table>
        <TableHead>
          <TableRow>
            <TableCell>
              <Typography variant="h6" style={{ color: "#01444a", textAlign: "center" }}>
                Date
              </Typography>
            </TableCell>
            <TableCell>
          ...
          {events.map((event) => (
            <TableRow key={event._id}>
              <TableCell style={{ borderBottom: "none", paddingTop: "0.3rem", paddingBottom: "0.3rem" }}>
                <Typography component="p" style={{ fontWeight: "bold", color: "#0a4849", textAlign: "center" }}>
                  {moment(new Date(event.createdAt)).format("l")}
                </Typography>
              </TableCell>
                ...
                (item) =>
                  item.id === event.emotion && (
                    <TableCell key={item.id} style={{ borderBottom: "none", paddingTop: "0.3rem", paddingBottom: "0.3rem" }}>
                      <Typography component="p" style={{ fontWeight: "bold", color: "#0a4849", textAlign: "center", fontSize: "1.5rem" }}>
                        {item.ansEmo}
                      </Typography>
                    </TableCell>
                  )
              )}
              {questions[1].answerOptions.map(
                (item) =>
                  item.id === event.task && (
                    <TableCell key={item.id} style={{ borderBottom: "none", paddingTop: "0.3rem", paddingBottom: "0.3rem" }}>
                      <Typography component="p" style={{ fontWeight: "bold", color: "#0a4849", textAlign: "center" }}>
